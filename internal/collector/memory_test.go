...
// readHeapInuseMB is the heap-in-use ceiling we treat as the working-set
...
// the working set stays bounded by the cap x per-key size.
func TestCollectorMemoryHighCardinalityBurst(t *testing.T) {
	...
	cases := []struct {
		...
		{"small burst", 200_000, 4_000, 100},
		{"large burst", 2_000_000, 16_000, 400},
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			t.Logf("%s: %d events through 6 collectors -> heap growth %.2f MB (peak %.2f MB)",
				c.name, c.events, growthMB, peakMB)
...
// sustained load for a few seconds. It rate-limits via a token loop
// instead of time.Sleep per event so the achieved rate matches the
// target on slower CI runners.
func TestCollectorMemorySustained100Kps(t *testing.T) {
	...
	cases := []struct {
		...
		{"50k/s for 1s", 50_000, 1 * time.Second},
		{"100k/s for 3s", 100_000, 3 * time.Second},
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			tickInterval := time.Second / time.Duration(c.targetRate/batchSize)

			var totalEvents int
			stopAt := time.Now().Add(c.duration)
			...
			achievedRate := float64(totalEvents*3) / elapsed.Seconds()
			peakMB := readHeapInuseMB()
			growthMB := peakMB - baseMB

			t.Logf("sustained (%s): %d events x 3 collectors in %v = %.0f events/s; heap growth %.2f MB",
				c.name, totalEvents, elapsed.Round(time.Millisecond), achievedRate, growthMB)

			if achievedRate < float64(c.targetRate)*0.8 {
				t.Logf("WARNING: achieved rate %.0f/s is far below %d target -- runner may be slow", achievedRate, c.targetRate)
			}
			if growthMB > memoryBudgetMB {
...
func TestCollectorConcurrentRecord(t *testing.T) {
	cases := []struct {
		...
		{"1 goroutine", 1, 200_000},
		{"4 goroutines", 4, 50_000},
		{"16 goroutines", 16, 100_000},
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			for g := 0; g < c.goroutines; g++ {
				go func(seed int) {
					defer wg.Done()
					for i := 0; i < c.eventsPerGoroutine; i++ {
						sc.record(makeSyscallEvent(
							fmt.Sprintf("p%04d", (seed*c.eventsPerGoroutine+i)%4000),
							uint32(i%100),
			...
			expected := uint64(c.goroutines * c.eventsPerGoroutine)
